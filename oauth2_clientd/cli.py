...
def parse_arguments(config: ConfigParser) -> argparse.Namespace:
    registrations = list(config.keys())
    try:
    ...
    # This is super hacky but I haven't found a better way to exclude positional arguments
    # from specific options without also checking for it manually.
...
def resolve_registration(config: ConfigParser, provider: str, loops=None):
...
def main() -> None:
    ...
    try:
        if verbose:
            print("Reading 'builtin-providers.conf' from package data", file=sys.stderr, end='')
        ...
        if verbose:
            print(" failed.  Defaults may be unavailable.", file=sys.stderr)
    ...
    if args.config:
        for configfile in args.config:
            if configfile == '' or configfile == '/dev/null':
                config = ConfigParser()
                if args.debug:
    ...
    if args.provider:
        ...
        try:
            ...
            raise FatalError('No default provider configured.')
